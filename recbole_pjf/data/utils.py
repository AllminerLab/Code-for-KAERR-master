...
def save_train_links(config, train_dataset):
    train_user_id = train_dataset.inter_feat['user_id']
    train_item_id = train_dataset.inter_feat['job_id']
    user_id2no = train_dataset.field2id_token['user_id']
    item_id2no = train_dataset.field2id_token['job_id']
    train_user_no = user_id2no[train_user_id].reshape(1, -1)
    train_item_no = item_id2no[train_item_id].reshape(1, -1)
    train_links = np.concatenate([train_user_no, train_item_no], axis=0)
    data_dir = config['data_path']
    if os.path.exists(os.path.join(data_dir, 'train_links.npy')):
        old_train_links = np.load(os.path.join(data_dir, 'train_links.npy'))
        if not (old_train_links == train_links).all():
            raise ValueError('The train links are not the same as before!')

    np.save(os.path.join(data_dir, 'train_links.npy'), train_links)
...
def data_preparation(config, dataset):
    ...
    save_train_links(config, train_dataset)
    ...
    if phase == 'train':
        ...
        if model_name != 'KAERR':
            ...
            if eval_strategy == "full":
                ...
                return KAERR_RNegSampleEvalDataLoader
