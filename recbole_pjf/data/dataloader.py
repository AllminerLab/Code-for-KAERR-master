...
class KAERR_RNegSampleEvalDataLoader(NegSampleDataLoader):
    """:class:`NegSampleEvalDataLoader` is a dataloader for neg-sampling evaluation.
    It is similar to :class:`TrainDataLoader` which can generate negative items,
    and this dataloader also permits that all the interactions corresponding to each user are in the same batch
    and positive interactions are before negative interactions.
    ...
    def __init__(self, config, dataset, sampler, shuffle=False):
        ...
        if (
            ...
            self.num_user = dataset.kg.number_of_nodes(config['USER_ENTITY_TYPE'])
            self.num_item = dataset.kg.number_of_nodes(config['ITEM_ENTITY_TYPE'])
            self.max_path_len = config['max_path_len']
            self.max_path_num = config['max_path_num']
            self.direct = -1 if sampler.phase[-1] == 'j' else 1
            if config['USE_PATH']:
                if self.direct >= 0:
                    self.user_id2no = dataset.field2id_token[dataset.uid_field]
                    self.item_id2no = dataset.field2id_token[dataset.iid_field]
                else:
                    self.user_id2no = dataset.field2id_token[dataset.iid_field]
                    self.item_id2no = dataset.field2id_token[dataset.uid_field]
                self.user_no2index = dataset.user_no2index
                self.item_no2index = dataset.item_no2index
                self.all_meta_path = dataset.all_meta_path
                self.all_path_length = dataset.all_path_length
                self.ui_path_indexs = dataset.ui_path_indexs
    ...
    def collate_fn(self, index):
        ...
        if (
            ...
            if self.config['USE_PATH']:
                path_type = 'user_item'
                paths = torch.zeros((len(cur_data), self.max_path_num, self.max_path_len), dtype=torch.int64)
                valid_path_length = torch.zeros((len(cur_data), self.max_path_num), dtype=torch.int64)
                valid_path_num = torch.zeros((len(cur_data)), dtype=torch.int64)
                batch_index = 0
                users = cur_data.user_id
                items = cur_data.job_id
                for u, i in zip(users, items):
                    user_no = self.user_id2no[u.item()]
                    item_no = self.item_id2no[i.item()]
                    user_index = self.user_no2index[user_no]
                    item_index = self.item_no2index[item_no]
                    ui_pair_index = (user_index * self.num_item + item_index)
                    path_index = self.ui_path_indexs[ui_pair_index]
                    path_index = path_index[path_index != 0]
                    path_index = list(path_index)
                    np.random.shuffle(path_index)
                    path_index = path_index[:self.max_path_num]
                    paths[batch_index, :len(path_index), :] = torch.tensor(self.all_meta_path[path_index])
                    valid_path_length[batch_index, :len(path_index)] = torch.tensor(self.all_path_length[path_index])
                    valid_path_num[batch_index] = len(path_index)
                    batch_index += 1
                cur_data.update(Interaction({f'{path_type}_paths': paths, f'{path_type}_valid_path_length': valid_path_length, f'{path_type}_valid_path_num': valid_path_num}))
            if self.bilateral_type == True:
                return cur_data, idx_list, positive_u, positive_i, positive_u_list
...
class KAERRTrainDataLoader(TrainDataLoader):
    def __init__(self, config, dataset, sampler, shuffle=False):
        super().__init__(config, dataset, sampler, shuffle=shuffle)
        self.used_ids_neg_i = self._sampler.used_ids
        self.change_field_direction()
    ...
    def _neg_sampling(self, inter_feat):
        inter_feat_neg_i = super(KAERRTrainDataLoader, self)._neg_sampling(inter_feat)

        self.change_field_direction()
        self._sampler.used_ids = self.used_ids_neg_u
        inter_feat_neg_u = super(KAERRTrainDataLoader, self)._neg_sampling(inter_feat)
        ...
        if self.config['USE_PATH']:
            for path_type in ['user_item', 'user_neg_item', 'neg_user_item']:
                paths = torch.zeros((len(inter_feat), self.max_path_num, self.max_path_len), dtype=torch.int64)
                valid_path_length = torch.zeros((len(inter_feat), self.max_path_num), dtype=torch.int64)
                valid_path_num = torch.zeros((len(inter_feat)), dtype=torch.int64)
                batch_index = 0
                if path_type == 'user_item':
                    users = inter_feat.user_id
                    items = inter_feat.job_id
                elif path_type == 'user_neg_item':
                    users = inter_feat.user_id
                    items = inter_feat.neg_job_id
                elif path_type == 'neg_user_item':
                    users = inter_feat.neg_user_id
                    items = inter_feat.job_id
                #import time
                #s = time.time()
                for u, i in zip(users, items):
                    ...
                    ui_pair_index = (user_index * self.num_item + item_index)
                    path_index = self.ui_path_indexs[ui_pair_index]
                    # numpy filter out zero
                    path_index = path_index[path_index != 0]
                    path_index = list(path_index)
                    np.random.shuffle(path_index)
                    path_index = path_index[:self.max_path_num]
                    paths[batch_index, :len(path_index), :] = torch.tensor(self.all_meta_path[path_index])
                    # for p in paths[batch_index]:
                    #      print('-'.join([self.kg_text[e.item()] for e in p if e != 0]))
                    # print(label)
                    valid_path_length[batch_index, :len(path_index)] = torch.tensor(self.all_path_length[path_index])
                    valid_path_num[batch_index] = len(path_index)
                    batch_index += 1
                #print(time.time()-s)
                inter_feat.update(Interaction({f'{path_type}_paths': paths, f'{path_type}_valid_path_length': valid_path_length, f'{path_type}_valid_path_num': valid_path_num}))
