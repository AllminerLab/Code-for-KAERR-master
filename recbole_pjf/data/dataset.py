    ...
    def _data_processing(self):
        ...
        if self.config['biliteral']:
            ...
            if self.direct_field:
                ...
                # 如果数据集只有单向交互，那么直接将user_single_inter和item_single_inter设置为inter_feat
        ...
        def bert_encoding(doc_list: list):
            ...
            for line in doc_list:
                for wd in line:
                    s += wd + ' '
            s = s[:512]
            input = self.tokenizer(s, return_tensors="pt").to(self.config['device'])
            output = self.model(**input)[0][:, 0].detach().cpu()
    ...
    def _doc_dataframe_to_interaction(self, data):
        ...
        for k in data:
            ...
            if k in [self.uid_field, self.iid_field]:
                ...
                new_data[k] = value
                #new_data[k] = torch.stack(value.tolist(), dim=1).transpose(0, 1)
...
class KAERRDataset(Dataset):
    """:class:`KAERRDataset` is inherited from :class:`recbole.data.dataset.Dataset`
    ...
    def _change_feat_format(self):
        super(KAERRDataset, self)._change_feat_format()
        if self.user_doc is not None:
            self._change_doc_format()

    def _data_processing(self):
        super(KAERRDataset, self)._data_processing()
        if self.user_doc is not None:
            self._doc_fillna()
    ...
    def build(self):
        ...
        Returns:
            list: List of built :class:`Dataset`.
        """
        datasets = super(KAERRDataset, self).build()
    ...
    def _get_field_from_config(self):
        """Initialization common field names.
        """
        super(KAERRDataset, self)._get_field_from_config()
        self.udoc_field = self.config['USER_DOC_FIELD']
        self.idoc_field = self.config['ITEM_DOC_FIELD']
        self.logger.debug(set_color('udoc_field', 'blue') + f': {self.udoc_field}')
        self.logger.debug(set_color('idoc_field', 'blue') + f': {self.idoc_field}')
    ...
    def _load_data(self, token, dataset_path):
        ...
        Args:
            ...
            dataset_path (str): path of dataset dir.
        """
        super(KAERRDataset, self)._load_data(token, dataset_path)
        if 'udoc' in self.config['load_col']:
            self.user_doc = self._load_user_or_item_doc(token, dataset_path, 'udoc', 'uid_field', 'udoc_field')
            self.item_doc = self._load_user_or_item_doc(token, dataset_path, 'idoc', 'iid_field', 'idoc_field')
            self.filter_data_with_no_doc()
        # load KG 
        self.kg = dgl.data.CSVDataset(dataset_path+'/kg/', force_reload=True)[0]
        # reverse edge
        #self.kg = reverse_kg(self.kg)
        self.df_user = pd.read_csv(dataset_path+'/table1_user_processed.txt', sep='\t')
        self.df_item = pd.read_csv(dataset_path+'/table2_jd_processed.txt', sep='\t')
        self.user_no2index = {no: i for i, no in enumerate(self.df_user['user_id'].values)}
        self.item_no2index = {no: i for i, no in enumerate(self.df_item['jd_no'].values)}

        # load text of KG for case study
        if self.config['CASE_STUDY']:
            self.kg_text = load_kg_text(self.kg, dataset_path+'/kg_text/')
            

        # load entity and relation embedding
        self.ntype_start_idx = {self.kg.ntypes[0]: 1}
        for i in range(1, len(self.kg.ntypes)):
            self.ntype_start_idx[self.kg.ntypes[i]] = self.ntype_start_idx[self.kg.ntypes[i-1]] + self.kg.number_of_nodes(self.kg.ntypes[i-1])
        if self.config['USE_KGE']:
            kg_emb_size = self.config['kg_embedding_size']
            self.kg_embedding = np.load(dataset_path+f'/kge/kg_embedding_{kg_emb_size}.npy')
            # for ntype in self.kg.ntypes:
            #     self.kg.nodes[ntype].data['feature'] = torch.from_numpy(
            #         self.kg_embedding[self.ntype_start_idx[ntype]:self.ntype_start_idx[ntype]+self.kg.number_of_nodes(ntype)]).float()
        
        # load kg path
        if self.config['USE_PATH']:
            mmap = 'r' if self.config['mmap'] else None
            self.all_meta_path = np.load(os.path.join(dataset_path, 'kg', 'meta_path', 'all_paths.npy'), mmap_mode=mmap)
            self.all_path_length = np.load(os.path.join(dataset_path, 'kg', 'meta_path', 'path_len.npy'), mmap_mode=mmap)
            self.ui_path_indexs = np.load(os.path.join(dataset_path, 'kg', 'meta_path', 'ui_path_index.npy'), mmap_mode=mmap)
    ...
    def join(self, df):
        ...
        Returns:
            Interaction: Interaction feature after joining operation.
        """
        df = super(KAERRDataset, self).join(df)
        if self.user_doc is not None and self.uid_field in df:
            df.update(self.user_doc[df[self.uid_field]])
        if self.item_doc is not None and self.iid_field in df:
            df.update(self.item_doc[df[self.iid_field]])
    ...
    def field2feats(self, field):
        """Overload code from :class:`recbole.data.dataset`
        """
        feats = super(KAERRDataset, self).field2feats(field)
        if field == self.uid_field:
            if self.user_doc is not None:
                feats.append(self.user_doc)
        elif field == self.iid_field:
...
def reverse_kg(kg):
    triple2rev = [triple for triple in kg.canonical_etypes if triple[0] == 'user']
    triple2rev = {triple: (triple[2], triple[1], triple[0]) for triple in triple2rev}
    new_data = {triple: kg.edges(etype=triple[1]) for triple in kg.canonical_etypes}
    new_keys = []
    new_values = []
    key2del = []
    for key, value in new_data.items():
        if key in triple2rev:
            new_value = (value[1], value[0])
            rev_key = triple2rev[key]
            new_keys.append(rev_key)
    ...
    for i in range(len(new_keys)):
        new_data[new_keys[i]] = new_values[i]
    
    return dgl.heterograph(new_data)

def load_kg_text(kg, kg_text_path):
    text_files = os.listdir(kg_text_path)
    entity_texts = {}
    for f in text_files:
        entity_type = f.split('.')[0].split('_')[-1]
        entity_texts[entity_type] = pd.read_csv(kg_text_path + f).iloc[:, 0].tolist()

    kg_text = [' ']
    for ntype in kg.ntypes:
        for text in entity_texts[ntype]:
            kg_text.append(str(text))

    for etype in kg.etypes:
        kg_text.append(etype)
    return kg_text